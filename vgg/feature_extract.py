...
if __name__ == '__main__':
    ...
    densenet_feature_extractor = VGG16(n_classes=25)  # 要提前知道你使用的网络在全连接层的输入维度即特征维度
    if use_gpu:
    ...
    del pretrained_dict["*.weight"]
    del pretrained_dict["*.bias"]
    model_dict.update(pretrained_dict)
    densenet_feature_extractor.load_state_dict(model_dict)
    densenet_feature_extractor.classifier = nn.Linear(25, 25) # 全连接层设置为25*25 并将weight设置为单位矩阵, 这样输出就是特征
